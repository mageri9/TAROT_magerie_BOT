...
async def can_get_card(user_id: int) -> bool:
    ...
    result = await db.fetchone(
        "SELECT last_card_date FROM user_cards WHERE user_id = $1", (user_id,)
    )
    if not result:
        return True
...
async def get_last_user_cards(user_id: int, limit: int = 10) -> list[int]:
    ...
    rows = await db.fetchall(
        "SELECT card_id FROM user_card_history WHERE user_id = $1 ORDER BY opened_at DESC LIMIT $2",
        (user_id, limit),
    )
...
async def add_card_to_history(user_id: int, card_id: int) -> None:
    ...
    await db.execute(
        "INSERT INTO user_card_history (user_id, card_id) VALUES ($1, $2)", (user_id, card_id)
    )
...
async def reset_daily_card_limit(user_id: int) -> None:
    ...
    await db.execute(
        ...
        VALUES ($1, '2000-01-01')
        ON CONFLICT (user_id)
        DO UPDATE SET last_card_date = EXCLUDED.last_card_date
        """,
        (user_id,),
    )
